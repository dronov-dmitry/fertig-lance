...
window.router.register('profile', async function(props) {
    ...
    const linkifyText = (value) => {
        ...
        while ((match = urlRegex.exec(text)) !== null) {
            ...
            const hrefRaw = url.startsWith('http://') || url.startsWith('https://') ? url : `https://${url}`;
            const href = encodeURI(hrefRaw).replace(/"/g, '%22');
            out += `<a href="${href}" target="_blank" rel="noopener noreferrer" style="color: var(--primary-color); text-decoration: underline;">${escapeHtml(url)}</a>${escapeHtml(trailing)}`;
    ...
    window.renderProfileView = () => {
        ...
        const currentFilter = window.profileState.reviewFilter || 'ALL';

        // Recalculate average ratings from current reviews in state for display
        const reviews = window.profileState.reviews || [];
        const workerRatings = reviews.filter(r => r.role === 'AS_WORKER').map(r => r.rating);
        const clientRatings = reviews.filter(r => r.role === 'AS_CLIENT').map(r => r.rating);
        
        const avgWorker = workerRatings.length > 0 ? (workerRatings.reduce((a, b) => a + b, 0) / workerRatings.length).toFixed(1) : '—';
        const avgClient = clientRatings.length > 0 ? (clientRatings.reduce((a, b) => a + b, 0) / clientRatings.length).toFixed(1) : '—';
        ...
        if (isEdit) {
            container.innerHTML = `
                <div class="card" style="max-width: 600px; margin: 0 auto;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                        ...
                        <button class="btn btn-outline" style="color: #2c3e50; border-color: #2c3e50;" onclick="window.toggleProfileEdit()">${tr('profile.cancel')}</button>
                    </div>
            ...
            const ratingWorker = avgWorker;
            const ratingClient = avgClient;
            ...
            container.innerHTML = `
                <div class="card" style="max-width: 600px; margin: 0 auto; text-align: center;">
                    ...
                    <div style="display: flex; flex-wrap: wrap; margin-bottom: 25px; border-top: 1px solid #f0f0f0; border-bottom: 1px solid #f0f0f0; padding: 15px 0; gap: 15px 0;">
                        <div style="flex: 1 1 33.333%; min-width: 100px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center;">
                            <div style="font-size: 1.5rem; font-weight: bold; color: var(--primary-color);">${ratingWorker}</div>
                            <div style="font-size: 0.75rem; color: #7f8c8d; text-transform: uppercase; line-height: 1.2;">${tr('profile.ratingWorker')}</div>
                        </div>
                        <div style="flex: 1 1 33.333%; min-width: 100px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center;">
                            <div style="font-size: 1.5rem; font-weight: bold; color: var(--secondary-color);">${ratingClient}</div>
                            <div style="font-size: 0.75rem; color: #7f8c8d; text-transform: uppercase; line-height: 1.2;">${tr('profile.ratingClient')}</div>
                        </div>
                        <div style="flex: 1 1 33.333%; min-width: 100px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center;">
                            <div style="font-size: 1.5rem; font-weight: bold; color: #27ae60;">
                                ${p.hourly_rate 
                                    ? (window.utils.formatCurrency ? window.utils.formatCurrency(p.hourly_rate) : p.hourly_rate.toLocaleString(locale) + ' ₽') 
                                    : '—'
                                }
                            </div>
                            <div style="font-size: 0.75rem; color: #7f8c8d; text-transform: uppercase; line-height: 1.2;">${tr('profile.perHour')}</div>
                        </div>
                    </div>

                    <div style="text-align: left; margin-bottom: 30px;">
                        <h3 style="margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 5px;">${tr('profile.aboutTitle')}</h3>
                        <p style="white-space: pre-wrap; color: #2c3e50; line-height: 1.6; word-break: break-word;">${p.bio ? linkifyText(p.bio) : escapeHtml(tr('profile.noBio'))}</p>
                        ${p.portfolio_url ? `<p style="margin-top: 10px; word-break: break-all;"><a href="${p.portfolio_url}" target="_blank" rel="noopener noreferrer" style="color: var(--primary-color); text-decoration: none; font-size: 0.95rem;">🔗 ${tr('profile.portfolioLink')}</a></p>` : ''}
                        ${p.telegram_url ? `<p style="margin-top: 8px; word-break: break-all;"><a href="${p.telegram_url}" target="_blank" rel="noopener noreferrer" style="color: var(--primary-color); text-decoration: none; font-size: 0.95rem;">📱 ${tr('profile.telegramLink')}</a></p>` : ''}
                    </div>
                    <div style="text-align: left; margin-bottom: 30px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-bottom: 15px; flex-wrap: wrap; gap: 10px;">
                        ...
                        <div style="margin-bottom: 15px; display: flex; flex-wrap: wrap; gap: 8px;">
                            <button class="btn ${currentFilter === 'ALL' ? 'btn-primary' : 'btn-outline'}" style="padding: 4px 12px; font-size: 0.85rem; ${currentFilter !== 'ALL' ? 'color: #2c3e50; border-color: #2c3e50;' : ''}" onclick="window.setReviewFilter('ALL')">${tr('profile.filterAll')}</button>
                            <button class="btn ${currentFilter === 'AS_WORKER' ? 'btn-primary' : 'btn-outline'}" style="padding: 4px 12px; font-size: 0.85rem; ${currentFilter !== 'AS_WORKER' ? 'color: #2c3e50; border-color: #2c3e50;' : ''}" onclick="window.setReviewFilter('AS_WORKER')">${tr('profile.filterAsWorker')}</button>
                            <button class="btn ${currentFilter === 'AS_CLIENT' ? 'btn-primary' : 'btn-outline'}" style="padding: 4px 12px; font-size: 0.85rem; ${currentFilter !== 'AS_CLIENT' ? 'color: #2c3e50; border-color: #2c3e50;' : ''}" onclick="window.setReviewFilter('AS_CLIENT')">${tr('profile.filterAsClient')}</button>
                        </div>
                        ...
                        <div id="inline-review-form-profile" style="display: none; margin-bottom: 20px; padding: 15px; border: 1px solid #f39c12; border-radius: 8px; background: #fffdf5;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1px 1fr; gap: 15px; margin-bottom: 15px; align-items: center;">
                                <div>
                                ...
                                <div style="background: #ddd; height: 40px;"></div>
                                <div>
                                    <label style="display: block; margin-bottom: 5px; font-weight: bold;">${tr('profile.reviewAsLabel') || 'Оставить отзыв как'}</label>
                                    <select id="review-role-select" class="form-control" style="width: 100%; padding: 8px; border: 1px solid #ced4da; border-radius: 4px;">
                                        <option value="AS_WORKER">${tr('profile.asClientAboutWorker') || 'Заказчик об исполнителе'}</option>
                                        <option value="AS_CLIENT">${tr('profile.asWorkerAboutClient') || 'Исполнитель о заказчике'}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div style="display: flex; gap: 10px; justify-content: flex-end;">
                                <button onclick="window.toggleReviewForm()" class="btn btn-outline" style="padding: 8px 15px; color: #2c3e50; border-color: #2c3e50;">${tr('profile.cancel')}</button>
                        ...
                        <div id="profile-reviews-list">
                            ${(() => {
                                let reviews = window.profileState.reviews || [];
                                if (currentFilter !== 'ALL') {
                                    reviews = reviews.filter(r => r.role === currentFilter);
                                }
                                
                                if (reviews.length === 0) {
                                    return '<p style="color: #95a5a6; font-style: italic;">' + tr('profile.noReviews') + '</p>';
                                }

                                return reviews.map(review => {
                                    const isMyReview = currentUserId && review.reviewer_id === currentUserId;
                                    const roleBadge = review.role === 'AS_WORKER' 
                                        ? `<span class="badge freelancer" style="font-size: 0.7rem; margin-left: 8px;">${tr('profile.roleWorker') || 'Исполнитель'}</span>`
                                        : `<span class="badge client" style="font-size: 0.7rem; margin-left: 8px;">${tr('profile.roleClient') || 'Заказчик'}</span>`;
                                    ...
                                    <div class="review-card" id="review-card-${review.review_id}" data-review-id="${review.review_id}" data-target-user-id="${p.user_id}" data-rating="${review.rating}" style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #f0f0f0;">
                                        ...
                                        <div id="review-edit-form-${review.review_id}" style="display: none; margin-top: 10px; padding: 12px; border: 1px solid #f39c12; border-radius: 8px; background: #fffdf5;">
                                            ...
                                            <div style="display: flex; gap: 10px; justify-content: flex-end;">
                                                <button type="button" onclick="window.cancelEditReview('${review.review_id}')" class="btn btn-outline" style="padding: 6px 12px; font-size: 0.9rem; color: #2c3e50; border-color: #2c3e50;">${tr('profile.cancel')}</button>
                    ...
                    ${isOwnProfile 
                        ...
                        : (window.auth.isLoggedIn() ? `
                            ...
                            <div id="inline-msg-form-profile" style="display: none; margin-top: 15px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background: #fafafa; text-align: left;">
                                ...
                                <div style="display: flex; gap: 10px; justify-content: flex-end;">
                                    <button onclick="window.toggleProfileMessageForm()" class="btn btn-outline" style="padding: 8px 15px; color: #2c3e50; border-color: #2c3e50;">${tr('profile.cancel')}</button>
    ...
    window.setReviewFilter = function(filter) {
        window.profileState.reviewFilter = filter;
        window.renderProfileView();
    };
    ...
    window.submitProfileReview = async function(targetUserId) {
        ...
        const roleEl = document.getElementById('review-role-select');
        
        const rating = parseInt(ratingEl.value, 10);
        const comment = textEl.value.trim();
        const role = roleEl ? roleEl.value : 'AS_WORKER';
        ...
        try {
            ...
            await window.api.submitProfileReview(targetUserId, rating, comment, role);
