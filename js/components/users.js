...
window.router.register('users', async () => {
    ...
    try {
        ...
        users.forEach(user => {
            ...
            // Only show review button if it's not the current user and they can actually review (worked together and no review yet)
            if (currentUserId && currentUserId !== user.user_id && !isBanned && user.can_review) {
                actionButtons += `
                    <button type="button" onclick="window.toggleUserReviewForm('${user.user_id}')" class="btn btn-outline" style="font-size: 0.8rem; padding: 5px 10px; color: #2c3e50; border-color: #2c3e50;">
                        ${tr('users.leaveReview')}
                    </button>
                `;
            }
            const reviewFormHtml = (currentUserId && currentUserId !== user.user_id && !isBanned && user.can_review) ? `
            ...
            usersHtml += `
                <div class="user-card card ${isBanned ? 'banned' : ''}" style="${isBanned ? 'opacity: 0.6;' : ''}">
                    <div class="user-header">
                        <h3><a href="#/profile?id=${user.user_id}" style="text-decoration: underline; color: var(--primary-color); cursor: pointer;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">${user.name || tr('users.noName')}</a> ${roleText ? `<span class="badge ${roleClass}">${roleText}</span>` : ''} ${banBadge}</h3>
        ...
        return `
            <div class="users-container">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; margin-top: 1rem; flex-wrap: wrap; gap: 15px;">
