// Create Task Module
(function() {
    function init() {
        const createTaskBtn = document.getElementById('createTaskBtn');
        const createTaskModal = document.getElementById('createTaskModal');
        const createTaskForm = document.getElementById('createTaskForm');
        
        if (!createTaskModal || !createTaskForm) {
            return;
        }

        // Populate specializations dropdown
        const specializationSelect = document.getElementById('taskSpecialization');
        if (specializationSelect) {
            const specializations = window.APP_CONFIG.specializations || [];
            const tr = window.i18n && window.i18n.t ? window.i18n.t.bind(window.i18n) : (k) => k;
            specializationSelect.innerHTML = `<option value="" disabled selected>${tr('users.allSpecializations') || 'Выберите специализацию'}</option>`;
            specializations.forEach(spec => {
                const option = document.createElement('option');
                option.value = spec;
                option.textContent = spec;
                specializationSelect.appendChild(option);
            });
        }

        // Set minimum date for deadline to tomorrow
        const tomorrow = new Date();
        tomorrow.setDate(tomorrow.getDate() + 1);
        const taskDeadlineInput = document.getElementById('taskDeadline');
        if (taskDeadlineInput) {
            taskDeadlineInput.min = tomorrow.toISOString().split('T')[0];
        }
        
        // Open modal on old header button click if it exists
        if (createTaskBtn) {
            createTaskBtn.addEventListener('click', () => {
                createTaskModal.classList.add('active');
            });
        }

        // Update budget placeholder based on type and currency
        const budgetInput = document.getElementById('taskBudget');
        const budgetTypeSelect = document.getElementById('taskBudgetType');
        const currencySelect = document.getElementById('taskCurrency');
        
        function updatePlaceholder() {
            const tr = window.i18n && window.i18n.t ? window.i18n.t.bind(window.i18n) : (k) => k;
            const type = budgetTypeSelect ? budgetTypeSelect.value : 'FIXED';
            const curr = currencySelect ? currencySelect.value : 'RUB';
            
            const symbols = { 'RUB': '₽', 'USD': '$', 'EUR': '€', 'UAH': '₴' };
            const symbol = symbols[curr] || '';
            
            if (type === 'HOURLY') {
                const base = tr('createTask.budgetHourlyPlaceholder') || '1000';
                budgetInput.placeholder = `${base} ${symbol}`;
            } else {
                const base = tr('createTask.budgetPlaceholder') || '5000';
                budgetInput.placeholder = `${base} ${symbol}`;
            }
        }

        if (budgetInput) {
            if (budgetTypeSelect) budgetTypeSelect.addEventListener('change', updatePlaceholder);
            if (currencySelect) currencySelect.addEventListener('change', updatePlaceholder);
            // Initial run
            updatePlaceholder();
        }
        
        // Close modal on X click
        const closeBtn = createTaskModal.querySelector('.close');
        if (closeBtn) {
            closeBtn.addEventListener('click', () => {
                createTaskModal.classList.remove('active');
            });
        }
        
        /* 
        // Close modal on outside click (only if it was a real click, not a selection release)
        let mouseDownTarget = null;
        createTaskModal.addEventListener('mousedown', (e) => {
            mouseDownTarget = e.target;
        });
        
        createTaskModal.addEventListener('mouseup', (e) => {
            if (e.target === createTaskModal && mouseDownTarget === createTaskModal) {
                createTaskModal.classList.remove('active');
            }
            mouseDownTarget = null;
        });
        */
        
        // Handle form submission
        createTaskForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const tr = window.i18n && window.i18n.t ? window.i18n.t.bind(window.i18n) : (k) => k;
            const title = document.getElementById('taskTitle').value.trim();
            const description = document.getElementById('taskDescription').value.trim();
            const budget = parseFloat(document.getElementById('taskBudget').value);
            const budgetTypeSelect = document.getElementById('taskBudgetType');
            const currencySelect = document.getElementById('taskCurrency');
            const budget_type = budgetTypeSelect ? budgetTypeSelect.value : 'FIXED';
            const currency = currencySelect ? currencySelect.value : 'RUB';
            const deadline = document.getElementById('taskDeadline').value;
            const specialization = document.getElementById('taskSpecialization').value;
            const skillsInput = document.getElementById('taskSkills').value.trim();
            if (!title || !description || !specialization) {
                window.utils.showToast(tr('createTask.fillRequired'), 'error');
                return;
            }
            if (budget <= 0) {
                window.utils.showToast(tr('createTask.budgetPositive'), 'error');
                return;
            }
            if (!deadline) {
                window.utils.showToast(tr('createTask.setDeadline'), 'error');
                return;
            }
            
            // Parse skills
            const skills = skillsInput 
                ? skillsInput.split(',').map(s => s.trim()).filter(s => s.length > 0)
                : [];
            
            // Prepare task data (backend expects category and budget_estimate)
            const taskData = {
                title,
                description,
                category: specialization, 
                budget_estimate: budget,
                budget_type: budget_type,
                currency: currency,
                deadline,
                skills
            };
            
            const submitBtn = createTaskForm.querySelector('button[type="submit"]');
            submitBtn.disabled = true;
            submitBtn.textContent = '...';
            try {
                const response = await window.api.createTask(taskData);
                window.utils.showToast(tr('createTask.created'), 'success');
                localStorage.removeItem('my-tasks-cache');
                localStorage.removeItem('my-tasks-cache-time');
                createTaskModal.classList.remove('active');
                createTaskForm.reset();
                window.router.navigate('my-tasks');
            } catch (error) {
                console.error('[CreateTask] Error:', error);
                window.utils.showToast(error.message || tr('createTask.createError'), 'error');
            } finally {
                submitBtn.disabled = false;
                submitBtn.textContent = tr('createTask.title');
            }
        });
    }
    
    // Initialize when DOM is ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', init);
    } else {
        init();
    }
})();
