...
class CurrencyService {
     ...
     * @param {number} amount Amount in some currency
     * @param {string} providedCurrency Optional specific currency (e.g., 'USD', 'EUR') to use instead of user preference
     * @returns {string} Formatted string
     */
    formatAmount(amount, providedCurrency = null) {
        if (amount === undefined || amount === null || isNaN(amount)) {
            return '—';
        }

        const preferred = providedCurrency || this.getPreferredCurrency();
        
        // Convert logic
        let convertedAmount = amount;
        
        // ONLY convert if we are NOT using a specifically provided currency 
        // AND the preferred currency is different from the base currency (RUB)
        if (!providedCurrency && preferred !== this.baseCurrency && this.rates && this.rates[preferred]) {
